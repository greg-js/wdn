...
// return true if regex matches at least one of the array items
...
module.exports = function() {
  ...
  complete.on('two', function() {
    var twoLastArgs = process.argv.slice(process.argv.length - 2)

    if (hasMatch(twoLastArgs, /ssh/)) {
      loadSshConfig(function(err, hosts) {
        if (err) { throw err; }
        complete.reply(hosts);
      });
    } else if (hasMatch(twoLastArgs, /^e(xec)?$|^remove$|^rm$|^s(how)?$/)) {
      var points = store('local').points();
      complete.reply(points);
    }
  });
